...
namespace Step64
{
  ...
  public:
    ...
    void run(const bool use_merged);
  ...
  private:
    ...
    void solve(const bool use_merged);
  ...
  template <int dim, int fe_degree>
  void HelmholtzProblem<dim, fe_degree>::solve(const bool use_merged)
  {
  ...
  template <int dim, int fe_degree>
  void HelmholtzProblem<dim, fe_degree>::run(const bool use_merged)
  {
...
int main(int argc, char *argv[])
{
  ...
  pcout << std::endl
...

      {
        ...
        helmholtz_problem.run(false);
      }
      {
        HelmholtzProblem<3, 3> helmholtz_problem;
        helmholtz_problem.run(true);
      }
